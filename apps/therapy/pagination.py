...
class EmotionalCheckInFilter(filters.FilterSet):
    ...
    def filter_has_symptoms(self, queryset, name, value):
        """Filter by presence of physical symptoms"""
        if value:
    ...
    def filter_has_coping(self, queryset, name, value):
        """Filter by use of coping strategies"""
        if value:
    ...
    def qs(self):
        ...
        if user.is_authenticated and user.gentle_mode:
            # Don't show very intense checkins in gentle mode
...
class CopingStrategyFilter(filters.FilterSet):
    ...
    def qs(self):
        ...
        user = self.request.user
        if user.is_authenticated and user.gentle_mode:
            # Limit difficulty in gentle mode
